...
function SubCategories() {
    ...
    const handleSubmit = async (formData: any) => {
        try {
            ...
            toast.success(response?.data?.message || `Category ${edit ? "updated" : "created"} successfully`);
            handelClose()
        } catch (error: any) {
    ...
    const handelEdit = (category: any) => {
        ...
        form.setValue("categoryId", category.categoryId.toString());
        setEdit(true);
        setOpen(true);
    };
                    ...
                    <div className="max-h-96 overflow-y-auto">
                        ...
                        {items.length === 0 && (
                            <div className="text-center py-8 text-muted-foreground">
                                No second subcategories yet. Click the button above to add one.
                            </div>
                        )}
                    </div>
                </CardContent>
